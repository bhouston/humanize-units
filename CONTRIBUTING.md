...
4. Run `pnpm check`. Push the branch and open a PR against **main** with a Conventional Commit title and `Closes #<issue-number>` in the description. Describe resulting behavior and validation. CI validates the title, branch name, and matching issue reference. Husky validates local commit messages. Hooks can be bypassed, so commit messages must always be reviewed.
